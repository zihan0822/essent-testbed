...
object SingleEvenFilterMain {
  def main(args: Array[String]): Unit = {
    if (args.size > 0) {
      if (!Driver(() => new SingleEvenFilter(UInt(width = 16)), "firrtl")(c => new SingleEvenFilterTests(c))) System.exit(1)
    } else {
      if (!Driver.run(() => new SingleEvenFilter(UInt(width = 16)), "./test_run_dir/playground.SingleEvenFilter/SingleEvenFilter")(c => new SingleEvenFilterTests(c))) System.exit(1)
    }
  }
}